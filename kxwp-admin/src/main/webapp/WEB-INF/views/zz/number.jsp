...
<div class="k_contaniner">
	...
	<div class="k_right">
		<div class="content">
			<!--查询等功能-->
			<form class="k_change" method="post" action="<%=basePath%>zz/msAccount/list.do">
				...
				<b class="k-b">
					...
					<option value="">请选择</option>
					 <c:forEach items="${msroles }" var="item">
				...
				<tbody>
				    ...
				    <c:forEach items="${masterStationAccounts }" var="item" varStatus="status">
						<tr>
							...
							<td>${item.mobile }</td>
							<td>
								<c:forEach items="${item.msRoles }" var="item2" varStatus="status">
								...
								<c:forEach items="${item.msRoles }" var="item2">
							...
							<td>
								...
								<a href="<%=basePath %>zz/msAccount/get.do?id=${item.id }" class="t_edit">编辑</a><br>
								</c:if>
	...
	<div class="k_breadcrumb">
		...
		<a href="<%=basePath%>zz/msAccount/dimission.do" class="Quit_number">离职账户</a>
		<a href="<%=basePath%>zz/msAccount/addInit.do" class="add_menu"><i class="icon-plus"></i>添加账号</a>
	</div>
	<!--面包屑导航结束-->
...
$(document).ready(function() {
	...
	$(".t_view").click(function(){
		...
		$.ajax({
	        url:"<%=basePath%>zz/msAccount/resetPassword.do?id="+id+"&mobile="+mobile,
