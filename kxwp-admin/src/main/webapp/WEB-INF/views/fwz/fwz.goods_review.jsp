...
<body>
	...
	<div class="k_contaniner">
		...
		<div class="k_right">
			<div class="content">
				...
				<!-- 待审核状态才能进行审核 -->
				<c:if test="${goods_detail.goodsStatus.name eq 'REVIEWING' }">
					<div class="row">
					       <button type="button" class="j-review btn btn-success btn-lg" version-no="${goods_detail.versionNO }" goods-id="${goods_detail.id }"  new-status="ONSALE">审核通过</button>
					       <button type="button" class="j-review btn btn-danger btn-lg" version-no="${goods_detail.versionNO }"  goods-id="${goods_detail.id }"  new-status="REJECT">拒绝通过</button>
					</div>
				</c:if>
				
				<!-- 上架后可以下架 -->
				<c:if test="${goods_detail.goodsStatus.name eq 'ONSALE' }">
					<div class="row">
					       <button type="button" class="j-review btn btn-danger btn-lg" version-no="${goods_detail.versionNO }"  goods-id="${goods_detail.id }"  new-status="OFFSALE">下架</button>
					</div>
				</c:if>
		...
		<div class="k_breadcrumb">
			...
			<a href="<%=contextPath %>/fwz/goods/gotoListGoods.do" class="add_menu">商品列表</a>
		</div>
		<!--面包屑导航结束-->
		<!--footer开始-->
	...
	seajs.use("fwz/goods/review", function(view) {
		view.init();
	});
	</script>
</body>
