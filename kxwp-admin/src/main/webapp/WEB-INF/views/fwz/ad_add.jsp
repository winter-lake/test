		...
		<div class="content">
			<div class="add_gg">
					...
					<div class="a-d" id="recommendedIdDiv">
						...
						<span id="recommendedIdSpan"><select id="recommendedId" name="recommendedId" ></select></span>
						<a href="#" title="商品id请到商品列表查看，供应商id请到供应商列表查看，品牌id请到品牌列表查看" alt="" class="question icon-question-sign"></a>
					</div>
					<div class="a-d">
						<i class="tx_red"></i>
						<span class="s-p">URL链接 :</span>
						<input type="text" id="url" class="validate[custom[url]] text-input">
					</div>
					<p class="int">
					...
					<div class="a-d">
						...
						<input type="text" placeholder="开始时间" id="startTime" class="Wdate validate[required]">
					</div>
					<div class="a-d">
						<i class="star icon-asterisk tx_red"></i>
						<span class="s-p">结束时间 :</span>
						<input type="text" placeholder="结束时间" id="endTime" class="Wdate validate[required]">
					</div>
					<h5 class="int">活动添加成功后不允许再编辑，请您仔细核对</h5>
					<div class="a-d">
						<i class="tx_red"></i>
						<span class="s-p">审核设置 :</span>
						<input type="checkbox" id="adStatus" checked="checked" style="position:relative;top:2p">
						立即提审
					</div>
					</form>
					<div class="a-d sub">
	...
	//初始化资源状态
		...
		$.ajax({
		    ...
		    success:function(data){
			    ...
			    $('#adLocation option:first').remove();
		    }
		})
		
		KXWP.set_start_end_time($("#startTime"),$("#endTime"),{
                endMaxToday:false,
                preDays:365
        },true);
