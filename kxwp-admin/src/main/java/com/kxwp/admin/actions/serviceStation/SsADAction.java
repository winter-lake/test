...
/**
 * 服务站广告管理 date: 2016年8月17日 上午11:28:47
 *
 * @author wangjun
 */
@Controller
@RequestMapping(value = "/fwz/ssAd")
public class SsADAction {
  @Autowired
  ...
  @ResponseBody
  public ExchangeData<List<ADConfig>> find(@RequestBody ADQuery adQuery,
    ...
    adQuery.setServiceStationId(userModel.getServiceStation().getId());
  ...
  public ModelAndView getInit(Long id, HttpServletRequest request) {
    ...
    try {
      ...
      if (adConfig.getPhotoList().size() != 0)
        request.setAttribute("photoList0",
            adConfig.getPhotoList().get(adConfig.getPhotoList().size() - 1));

      mav.setViewName("fwz/ad_view");
      mav.addObject("id", id);
    } catch (Exception e) {
      mav.setViewName("500");
    }
