define('gys/goods/edit', function(require, exports, module) {
			...
			module.exports = {
						...
						if(check){
							...
							Model.edit(ajax_data).done(function(data) {
								if (data.callStatus == 'SUCCESS') {
									Common.kxpw_tishi("编辑商品成功");
									window.location.href="/gys/goods/gotoListGoods.do";
								}
							})
						}
					})
				...
				initLadder: function() {
					...
					ladder.on('click', '.j_add_ladder', function() {
						...
						var html = '<div class="j_ladder_item"><i>采购量 :<input type="text" name="minQit" class="validate[custom[number]]"> 至 <input type="text" name="maxQit" class="validate[custom[number]]"></i><i>梯度价 :<input type="text" name="lotPrice" class="validate[custom[number]]"> 元 </i><a href="javascript:void(0);" class="j_del_ladder">删除</a></div>';
						ladder.append(html);
					});
					...
					if(lotsPrice == 'Y'){
						...
						ladder.find('.j_ladder_item').each(function(index, el) {
							...
							if(minQit && maxQit && lotPrice) {
								...
								if(index == 0){
									if(_minQit != 1){
										Common.kxpw_tishi('阶梯数量设置不正确,数量必须从1开始');
									}
								}
				...
				init : function() {
					...
					// 初始化表单验证
					$('#edit_goods_form').validationEngine();

					// 点击查看大图
					Common.initProp(".w_view_pic img");
				}
			};
		});