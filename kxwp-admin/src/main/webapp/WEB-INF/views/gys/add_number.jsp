...
<div class="k_contaniner">
	...
	<div class="k_right">
		<div class="content">
			<form class="form_number" method="post" id="form_menu" action="<%=basePath%>gys/suAccount/add.do">
				<h4>基本信息</h4>
...
$(document).ready(function() {
	$(".icon-chevron-down").click(function(){
		...
		$.ajax({
			type:"POST",
			url:"/gys/suAccount/previewResourceInfo.do",
			dataType:"json",
			data:JSON.stringify(suRoles),
			...
			success:function(response){
				...
				$.each(response, function(i, n){
					  html+='<li class="j_haschild module active"><a href="#"><i class="icon-sitemap tx_16 mr10"></i><span class="name">'+n.name+'</span></a>';
					  		html+='<ul>';
					  	$.each(n.suResources, function(k, v){
    ...
    $(".yes").click(function(){
  	...
  	if(!$("input[name=mobile]").val().match(/^(13[0-9]|14[0-9]|15[0-9]|18[0-9])\d{8}$/)){
       ...
       return false;
	  	  ...
	  	  $.ajax({
	  		  type:'POST',
	  		  url:"/gys/suAccount/checkMobileIsUsed.do",
	  		  dataType:"json",
