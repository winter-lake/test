...
<head> 
	...
	<link rel="stylesheet" href="<%=basePath %>css/gys/role.css">
...
<div class="k_contaniner">
	...
	<!--右边开始-->
	<div class="k_right">
		<form action="<%=basePath %>gys/suRoleManage/list.do" method="post">
				...
				<tbody>
					<c:forEach items="${suRoles }" var="item" varStatus="status">
						<tr>
							...
							<a href="<%=basePath %>gys/suRoleManage/get.do?id=${item.id }" class="t_edit">编辑</a>
							<a href="#" class="t_del" id="${item.id }">删除</a>
							</c:if>
							<a href="<%=basePath %>gys/suRoleManage/view.do?id=${item.id }" class="t_view">查看</a>
							</td>
						</tr>
					</c:forEach>
				</tbody>
	...
	<div class="k_breadcrumb">
		...
		<a href="<%=basePath %>gys/suRoleManage/addInit.do" class="add_menu"><i class="icon-plus"></i>添加角色</a>
	</div>
	<!--面包屑导航结束-->
	<!--footer开始-->
...
<script type="text/javascript">
	$(function(){
		$(".t_del").click(function(){
			...
			$.ajax({
		    	url:"<%=basePath %>gys/suAccount/checkRoleIsUsed.do",
		    	type:"post",
		    	dataType:"json",
		    	data:{id:id},
		    	success:function(response){
		    		if(response == true){
		    			...
		    			$.ajax({
		    		    	url:"<%=basePath %>gys/suRoleManage/remove.do",
		    		    	type:"post",
		    		    	dataType:"json",
		    		    	data:{id:id},
		    		    	success:function(response){
