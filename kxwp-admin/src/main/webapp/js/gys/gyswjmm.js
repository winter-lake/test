	...
	Common.validator($('#wjmmform'));
	   ...
	   $(".hqyz").click(function(){
	    ...
	    if(mobile!="" && mobile.length==11 && Common.checkPhone(mobile)){
	    	$.ajax({
				url:"/gys/userManage/selectGysMobileAlive.do?mobile="+mobile,
				type:'GET',
				dataType:'json',
				success:function(data)
				{
					if(data.callStatus == 'SUCCESS'){
						Common.hqyzm($("#fsyzm"));
						$.ajax({
							url:"/gys/userManage/gysgetSMSCode.do?mobile="+mobile,
					        type:'GET',
					        dataType:'json',
					        success:function(data){
					        	if(data.callStatus == 'SUCCESS'){
	    ...
	    $(".qdxg").click(function(){
			...
			if(Common.checkPhone(mobile) && smscode!="" && pattern.test(newpwd) && pattern.test(repwd)){
				$.ajax({
					url:"/gys/userManage/gyscheckSMSCode.do?mobile="+mobile+"&SMScode="+smscode,
			        type:'GET',
			        dataType:'json',
			        success:function(data){
			        	if(data.callStatus == 'SUCCESS'){
