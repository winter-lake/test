...
<div class="k_contaniner">
	...
	<!--右边开始-->
	<div class="k_right">
		<form action="<%=basePath %>zz/msRoleManage/list.do" method="post">
				...
				<tbody>
					<c:forEach items="${msroles }" var="item" varStatus="status">
						<tr>
							...
							<td>
							<a href="<%=basePath %>zz/msRoleManage/get.do?id=${item.id }" class="t_edit">编辑</a>
							<a href="#" class="t_del" id="${item.id }">删除</a>
							<a href="<%=basePath %>zz/msRoleManage/view.do?id=${item.id }" class="t_view">查看</a>
							</td>
						</tr>
					</c:forEach>
				</tbody>
	...
	<div class="k_breadcrumb">
		...
		<a href="<%=basePath %>zz/msRoleManage/addInit.do" class="add_menu"><i class="icon-plus"></i>添加角色</a>
	</div>
	<!--面包屑导航结束-->
...
<script type="text/javascript">
	$(function(){
		$(".t_del").click(function(){
			...
			$.ajax({
		    	url:"<%=basePath %>zz/msAccount/checkRoleIsUsed.do",
		    	type:"post",
		    	dataType:"json",
		    	data:{id:id},
		    	success:function(response){
		    		if(response == true){
		    			...
		    			$.ajax({
		    		    	url:"<%=basePath %>zz/msRoleManage/remove.do",
		    		    	type:"post",
		    		    	dataType:"json",
		    		    	data:{id:id},
		    		    	success:function(response){
