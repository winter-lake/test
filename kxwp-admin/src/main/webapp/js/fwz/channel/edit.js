define('fwz/channel/edit', function(require, exports, module) {
			...
			module.exports = {
				...
				getSubmitData : function() {
					...
					$("#tbody tr").each(function(){
						...
						channelItem.id = $(this).find("td:eq(6)").text();
				...
				init : function() {
				    ...
				    $.ajax({
				    	type:"get",
				    	url:"/fwz/channel/ajax/getChannel.do?channelId="+$("#channelId").val(),
				    	dataType:"json",
				    	data:"",
				    	contentType:"text/html",
				    	success:function(response){
				    		console.log(response);
				    		$("#channelNameDesc").text(response.data.channelNameDesc);
				    		$("#platformTypeDesc").text(response.data.platformTypeDesc);
				    		$("#channelDesc").text(response.data.channelDesc);
				    		
				    		$("#channelName").val(response.data.channelName);
				    		$("#platformType").val(response.data.platformType);
				    		
				    		var html = "";
				    		
				    		$.each(response.data.channelItems, function(i, n){
				    			html += "<tr>"+
								"<td hidden=true>"+n.supplierId+"</td>"+
								"<td hidden=true>"+n.itemType+"</td>"+
								"<td hidden=true>"+n.itemId+"</td>"+
					            "<td>"+n.goodsName+"</td>"+
					            "<td>"+n.goodsPrice+"</td>"+
					            "<td>"+n.promotionPrice+"</td>"+
					            "<td hidden=true>"+n.id+"</td>"+
					            '<td><button type="button" class="btn btn-link btn-xs" name="delete">删除</button></td>"'+
					          "</tr>";
				    			});
