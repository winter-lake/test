...
<div class="k_contaniner">
	...
	<div class="k_right">
		<div class="content">
			<form class="add_form_role" method="post" id="form_menu" action="<%=basePath %>gys/suRoleManage/modify.do">
				<input type="hidden" name="id" value="${suRole.id }"/>
				<div class="div">
					<span>角色名 :</span>
				    <input type="text" name="name" value="${suRole.name }" class="validate[required]" placeholder="角色名" style="width:250px;border:1px solid #eee;height:30px;" data-required>
				</div>
				<div class="div">
					<span>角色描述 :</span> 
				    <textarea name="roleDescription" cols="30" rows="10" placeholder="角色描述" style="width:250px;border:1px solid #eee" data-required>${suRole.roleDescription }</textarea>
				</div>
				<h4>权限选择</h4>
				<div class="div">
				...
				<c:forEach items="${newlist }" var="item" varStatus="status">
					<div class="div">
						...
						<c:if test="${iscontain == true}">
							<p class="b_em"><input type="checkbox" checked="checked" name="suResources[${status.index }].id" value="${item.id }"> ${item.name }</p> 
						</c:if>
						...
						<c:forEach items="${item.suResources }" var="item2" varStatus="status2">
							...
							<c:if test="${iscontain0 == true }">
								<p><input type="checkbox" checked="checked" name="suResources[${status.index }].suResources[${status2.index }].id" value="${item2.id }"> ${item2.name }</p>
							</c:if>
