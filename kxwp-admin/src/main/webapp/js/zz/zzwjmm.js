	...
	Common.validator($('#wjmmform'));
	   ...
	   $(".hqyz").click(function(){
	    ...
	    if(mobile!="" && mobile.length==11 && Common.checkPhone(mobile)){
	    	$.ajax({
				url:"/zz/userManage/selectMobileAlive.do?mobile="+mobile,
				type:'GET',
				dataType:'json',
				success:function(data)
				{
					if(data.callStatus == 'SUCCESS'){
						Common.hqyzm($("#fsyzm"));
						$.ajax({
							url:"/zz/userManage/getSMSCode.do?mobile="+mobile,
					        type:'GET',
					        dataType:'json',
					        success:function(data){
					        	if(data.callStatus == 'SUCCESS'){
	    ...
	    $(".qdxg").click(function(){
			...
			if(Common.checkPhone(mobile) && smscode!="" && pattern.test(newpwd) && pattern.test(repwd)){
				$.ajax({
					url:"/zz/userManage/checkSMScode.do?mobile="+mobile+"&SMScode="+smscode,
			        type:'GET',
			        dataType:'json',
			        success:function(data){
			        	if(data.callStatus == 'SUCCESS'){
